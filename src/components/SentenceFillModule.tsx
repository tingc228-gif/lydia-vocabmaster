...
import { BookOpenText, CheckCircle2, ChevronRight, RotateCcw, XCircle } from 'lucide-react';
...
export default function SentenceFillModule({
  ...
  const questions = data.vocabularyInContextQuestions;
    ...
    return (
      <div className="studio-card">
        <div className="module-header">
          <div>
            <span className="module-pill">Vocabulary in Context</span>
            <h2 className="mt-4 text-4xl font-semibold">No reading questions yet</h2>
            <p className="module-subcopy mt-3">
              Generate a word set first, and this module will prepare short exam-style passages.
            </p>
          </div>
        </div>
      </div>
  ...
  if (isFinished) {
    ...
    return (
      <div className="celebration-panel studio-card">
        ...
        <h2 className="text-5xl font-semibold">Vocabulary in Context complete</h2>
        <p className="module-subcopy mx-auto mt-4 max-w-2xl">
          You kept revisiting the hard passages until every one was correct. That is strong exam-style practice.
        </p>
    ...
    <div className="grid gap-5 lg:grid-cols-[0.56fr_1.44fr]">
      <aside className="studio-card">
        <div className="module-header">
          <div>
            <span className="module-pill">Vocabulary in Context</span>
            <h2 className="mt-4 text-4xl font-semibold">Read the passage and infer the meaning</h2>
            <p className="module-subcopy mt-3">
              This is closer to a real exam question, and any wrong passage will come back again until it is solved.
            </p>
          </div>
        </div>
        ...
        <div className="mt-8 grid gap-4">
          ...
          <div className="studio-panel">
            ...
            <p className="muted-copy">Look at what happens before and after the target word. The answer should match the whole passage.</p>
          </div>
        </div>
      </aside>

      <section className="studio-card">
        <div className="rounded-[30px] border border-[rgba(209,174,214,0.35)] bg-[linear-gradient(180deg,rgba(255,255,255,0.94),rgba(255,245,250,0.92))] px-7 py-7 shadow-[var(--shadow-md)]">
          <p className="eyebrow">Passage</p>
          <p className="mt-4 text-2xl leading-[1.9] text-[#9f658c]">{currentQuestion.passage}</p>
        </div>

        <div className="mt-6 module-header">
          <div>
            <span className="module-pill">Current challenge</span>
            <h3 className="mt-4 text-3xl font-semibold leading-tight">{currentQuestion.question}</h3>
          </div>
        </div>
